...
library(stringr)
label_files <- list.files("/Volumes/immucan_volume/processed_data/Panel_1/2022_WORKFLOW/IMC/Rout/gated_cells/", 
                          full.names = TRUE, pattern = ".rds$")

# Read in SPE objects
spes <- lapply(label_files, readRDS)
names(spes) <- list.files("/Volumes/immucan_volume/processed_data/Panel_1/2022_WORKFLOW/IMC/Rout/gated_cells/", pattern = ".rds$")
spes <- spes[-108]

# Read in current SPE object
spe <- readRDS("/Volumes/immucan_volume/processed_data/Panel_1/2022_WORKFLOW/IMC/Rout/sce.rds")

new_spes <- lapply(1:length(spes), function(x){
        print(x)
        x <- spes[[x]]
...
lapply(1:length(new_spes), function(x){
    saveRDS(new_spes[[x]], file = paste0("/Volumes/immucan_volume/processed_data/Panel_1/2022_WORKFLOW/IMC/Rout/gated_cells/", names(spes)[x]))
})
