...
cur_sces <- lapply(seq_along(cur_files), function(x){
  ...
  colData(cur_sce) <- as(merge(as.data.frame(colData(cur_sce)), 
                            as.data.frame(cur_data_2)[,c("cell.ID", "cell.area", "TLS.ID")], by = "cell.ID",
...
cur_sces <- lapply(cur_files, function(x){
  ...
  colData(cur_sce)$sample_id <- str_extract(x, "100.*00[1-9]{1}")
  ...
  # Sanity check that phenotypes match
  stopifnot(all.equal(cur_sce$phenotype, cur_df$phenotype))
  
  # Transfer cell area and TLS id
  colData(cur_sce) <- merge(colData(cur_sce), cur_df[,c("cell.ID", "cell.area", "TLS.ID")], by = "cell.ID", sort = FALSE)
