...
p
ggsave(filename = "../output/Figure_S5/FigS5_A.pdf", plot = p, width = 10, height = 7)
```
...
p <- cur_dat %>%
  ...
  annotate("text", x = 0.009, y = 392 , label = expression( paste("IMC, mean = 78 mm"^2)), color = "blue", size = 5)+
  annotate("text", x = 0.01, y = 400 , label = expression( paste("mIF, mean = 149 mm"^2)), color = "red", size = 5)+
  ylab("cell size")+
  xlab("density estimate")
  #geom_hline(yintercept = mean_IMC, col= "blue")+
  #geom_hline(yintercept = mean_IF, col= "red")
p
ggsave(filename = "../output/Figure_S5/FigS5_B.pdf", plot = p, width = 7, height = 5)
```
...
p
ggsave(filename = "../output/Figure_S5/FigS5_C.pdf", plot = p, height = 5, width = 17)
```
