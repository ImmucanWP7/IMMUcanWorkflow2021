...
In this script we only load the pretrained classifier to generated Figure S5. For the full training please check the code in the repo above
...
p <- data.frame(cm$byClass) %>%
  ...
  geom_point(aes(1 - Specificity, Sensitivity, 
                 ...
                 fill = class),
             shape = 21) + 
  scale_fill_manual(values = cell_types) + 
...
ggsave (filename = "Fig_S5C.pdf", plot = p, device = "pdf", path = output, width = 7.5, height = 6.5)
```
...
p <- cur_pred %>%
  ...
  xlab("cell type")
p
#ggsave (filename = "Fig_S5D.png", plot = p, device = "png", path = output, width = 35, height = 22,units = "cm",dpi = 1200)
```
