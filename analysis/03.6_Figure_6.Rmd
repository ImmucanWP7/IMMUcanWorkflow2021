...
```{r}
getwd()
if (!dir.exists("output/Figure_6/")) dir.create("output/Figure_6/")
...
ggsave(filename = "output/Figure_6/Figure_6A.pdf", plot = p_final, width = 15, height = 10)
ggsave(filename = "output/Figure_6/Figure_6A_legend.pdf", plot = p_4_with_legend)
...
ggsave(filename = "output/Figure_6/Figure_6B.pdf", plot = p, width = 9, height = 6)
...
# CD11c pos vs DCs
cur_dat <- as.data.frame(colData(sce_mIF))
cur_dat <- cur_dat %>%
  mutate(DC = grepl("CD11c\\+", phenotype)) %>%
  group_by(sample_id) %>%
  summarize(count = sum(DC),
            total_cells = n()) %>%
...
frac_IMC <- cur_IMC %>% as_tibble() %>%
  ...
  filter(matched_celltype == "DC") %>%
  select(-matched_celltype)

combined <- left_join(frac_IMC, cur_dat, by=c("sample_id"))
combined$patient_id <- sub("-", "", str_extract(combined$sample_id, "[0-9]{8}-"))

(p_CD11c <- ggplot(combined) +
  ...
  theme_classic(base_size = 20) + xlab("IMC") + ylab("mIF") + 
  ...
  ggtitle("CD11c_pos vs DC") +
  coord_fixed())


# CD15 pos vs Neutrophils
cur_dat <- as.data.frame(colData(sce_mIF))
cur_dat <- cur_dat %>%
  mutate(DC = grepl("CD15\\+", phenotype)) %>%
  group_by(sample_id) %>%
  summarize(count = sum(DC),
            total_cells = n()) %>%
  mutate(frac = count/total_cells) %>%
...
frac_IMC <- cur_IMC %>% as_tibble() %>%
  ...
  summarize(count = n()) %>%
  mutate(frac = count/sum(count)) %>%
  filter(matched_celltype == "Neutrophil") %>%
  select(-matched_celltype)

combined <- left_join(frac_IMC, cur_dat, by=c("sample_id"))
combined$patient_id <- sub("-", "", str_extract(combined$sample_id, "[0-9]{8}-"))

(p_CD15 <- ggplot(combined) +
  stat_smooth(aes(frac.x, frac.y, color = patient_id), alpha = 0.5, se = FALSE, method='lm', formula = y~x, fullrange=FALSE, geom = "line", lwd = 1.5) + 
  geom_smooth(aes(frac.x, frac.y), se = TRUE, method='lm', formula = y~x, fullrange=TRUE, lwd = 1.5) + 
  geom_point(aes(frac.x, frac.y, color = patient_id), size = 3) + 
  geom_abline(slope = 1, intercept = 0, color = "dark red", lwd = 1.5) +
  ...
  theme(legend.position = "none",
        axis.text = element_text(size = 20)) +
  ggtitle("CD15_pos vs Neutrophil") +
  coord_fixed())
# some sanity checks
...
plot_grid(p_CD11c,p_DC)
plot_grid(p_CD15,p_Neutro)
```
...
ggsave(filename = "output/Figure_6/Figure_6C.pdf", plot = p_final, width = 30, height = 15)
```
...
ggsave(filename = "output/Figure_6/Figure_6D.pdf", plot = p, width = 12, height = 7)
...
all_sce$sample_id <- as.factor(all_sce$sample_id)
all_sce$modality <- as.factor(all_sce$modality)
# note: the .format_data function from spicyR has some lines that specifically call the imageIDCol with "imageID". We will therefore use this as column name
all_sce$imageID <- all_sce$sample_id_global
...
spicyTest <- spicy(
  ...
  condition = "modality",
  subject = "sample_id",
  spatialCoordCols = c("Pos_X", "Pos_Y"),
  window = "square",
...
ggsave(filename = "output/Figure_6/Figure_6E_new_2025Feb.pdf", plot = p, width = 7, height = 5)
```
