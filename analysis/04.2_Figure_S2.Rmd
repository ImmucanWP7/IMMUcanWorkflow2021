...
for(i in c("HLADR","GrzB","PDL1","Ecad","LAG3","PD1","Ki67","CD8a","CD4", "CD206","CD20","ICOS")){
  p.list[[i]] <- plotUMAP(cyto_sce, colour_by = i, by_exprs_values = "scaled", 
                          point_size = 0.25, point_alpha = 1, shape_by = "Activation_status") +
    scale_colour_gradient2(name = i, low = "#2166ac", mid = "white", high = "#b2182b",
                           limits = c(-3, 3), oob = squish)
}

png(file = "../output/Figure_S2/Figure_S2_UMAPmarkers.png",width = 1500 ,height = 1400)
...
p <- expr_df_act %>%
  ...
  theme_bw()+
  annotate(geom = "text",label = paste0("R2 =", round(r_square,digits = 2)),x = 3, y = 0.25)
