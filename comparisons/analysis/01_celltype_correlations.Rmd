...
```{r celltype-fractions, fig.width=12, fig.height=16}
# Match cell types
...
combined_df$patient_id <- sub("-", "", str_extract(combined_df$sample_id, "[0-9]{8}-"))
combined_df$ROI <- sub("_", "", str_extract(combined_df$sample_id, "_00[1-9]"))
...
ggplot(combined_df) + 
  ...
  theme_classic(base_size = 20) + xlab("Fraction cell types in IMC") + ylab("Fraction cell types in mIF") + 
  facet_wrap(.~matched_celltype, scales = "free") +
  scale_color_manual(values = metadata(sce_IMC)$color_vectors$patient_id)

ggplot(combined_df) + 
  geom_smooth(aes(frac.x, frac.y, color = patient_id),  se = FALSE, method='lm', formula= y~x, fullrange=TRUE) + 
  geom_point(aes(frac.x, frac.y, color = ROI)) + 
  geom_abline(slope = 1, intercept = 0, color = "dark red") +
  theme_classic(base_size = 20) + xlab("Fraction cell types in IMC") + ylab("Fraction cell types in mIF") + 
  facet_wrap(.~matched_celltype, scales = "free") +
...
# MA plot
ggplot(DA) + geom_point(aes(logCPM, logFC, color = sign), size = 3) +
  geom_label_repel(aes(logCPM, logFC, label = celltype), size = 5) + 
...
for (i in unique(sce_IMC$sample_id)) {
  ...
  cur_IMC <- plotPixels(images_IMC[i], 
    ...
    pdf(file = paste0("/Volumes/GoogleDrive/My Drive/IMMUcan_workflow/data/comparisons/images_celltypes/", i, ".pdf"))
...
# Write out legend
pdf(file = paste0("/Volumes/GoogleDrive/My Drive/IMMUcan_workflow/data/comparisons/images_celltypes/legend.pdf"))
